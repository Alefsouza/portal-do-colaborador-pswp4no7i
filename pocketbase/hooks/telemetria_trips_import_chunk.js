...
// de dados. Isso permite que cada requisicao reconstrua o headerMap e seja
// autocontida (mesma logica do hook telemetria_trips_import.js).
//
// Regras:
//  1. chunk_index == 0 => limpa todos os registros de telemetria_trips cujo
//     campo `data_viagem` seja igual a data_referencia (ou a data de hoje).
//  2. Faz o parse/insert usando a MESMA logica de header mapping
//     (headerToFieldMap, tripsFieldNames), normalizacao de nome, match de
//     motorista e extracao de data_viagem do hook telemetria_trips_import.js.
...
routerAdd('POST', '/backend/v1/telemetria/trips-import-chunk', (e) => {
  ...
  if (chunkIndex === 0) {
    try {
      $app
        .db()
        .newQuery('DELETE FROM telemetria_trips WHERE data_viagem = {:d}')
        .bind({ d: dataReferencia })
        .execute()
    } catch (err) {
      $app.logger().error('telemetria trips-import-chunk delete error', 'error', String(err))
    }
  }

  // ---- Parse do cabecalho + linhas (igual ao hook original) ----
  var lines = csvText.split('\n')
  if (lines.length < 2) {
    return e.json(400, { error: 'csv_chunk sem dados.' })
  }
  ...
  // ---- headerToFieldMap (igual ao hook original) ----
  ...
  for (var k = 0; k < dataLines.length; k++) {
    ...
    try {
      ...
      $app
        .logger()
        .error('telemetria trips-import-chunk save error', 'line', k + 2, 'error', String(err))
    }
  }
    ...
    $app
      ...
      .newQuery(
        'SELECT COUNT(*) as total, ' +
          ...
          'FROM telemetria_trips WHERE data_viagem = {:d}',
      )
      .bind({ d: dataReferencia })
      .one(sumModel)
  } catch (err) {
    $app.logger().error('telemetria trips-import-chunk summary error', 'error', String(err))
  }

  $app
    .logger()
    .info(
      'telemetria trips-import-chunk complete',
      'data',
      dataReferencia,
      'total',
      sumModel.total,
      'drivers_found',
      sumModel.encontrados,
      'drivers_not_found',
      sumModel.nao_encontrados,
    )
